...
import { StorageProvider, StorageProviderApi, StorageProviderApiOptionsIndexMap, AWSS3Provider, StorageProviderWithCopy, S3ProviderGetOuput, S3ProviderRemoveOutput, S3ProviderListOutput, S3ProviderCopyOutput, S3ProviderPutOutput, S3ProviderGetPropertiesOutput, StorageProviderWithGetProperties } from '../';
type ConfigParameter<F extends (...args: any) => any, U extends StorageProviderApi> = Parameters<F>[StorageProviderApiOptionsIndexMap[U]];
 ...
 * config.
 */
type StorageOperationConfig<T extends StorageProvider | StorageProviderWithCopy | StorageProviderWithGetProperties, U extends StorageProviderApi> = ReturnType<T['getProviderName']> extends 'AWSS3' ? ConfigParameter<AWSS3Provider[U], U> : T extends StorageProviderWithGetProperties & StorageProviderWithCopy ? ConfigParameter<T[U], U> & {
    provider: ReturnType<T['getProviderName']>;
} : T extends StorageProviderWithCopy ? ConfigParameter<T[Exclude<U, 'getProperties'>], U> & {
    provider: ReturnType<T['getProviderName']>;
} : T extends StorageProviderWithGetProperties ? ConfigParameter<T[Exclude<U, 'copy'>], U> & {
    provider: ReturnType<T['getProviderName']>;
